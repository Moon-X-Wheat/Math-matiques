...
### 0111 - 0115 导数和图像
函数的极值
- 什么是全局最大值、最小值；局部最大值、最小值
- 如何用导数求局部极极值：在一个开区间内，局部极值一定出现在临界点，也就是导数为0或者导数不存在的地方（反之不成立）；在一个闭区间内，极值可能出现在临界点或者端点。
罗尔定理：一个可导函数，在函数值相等的两点内，至少存在一个点导数为0。水平线如果连续经过函数两次，至少与函数相切一次。
中值定理：一个可导函数，两点内至少存在一点导数值等于两点的均值。两点的割线至少与函数相切一次。
二次导数和函数图像
- 如果二次导数大于0，函数开口向上；二次导数小于0，函数开口向下。
- 拐点：函数在拐点两侧，二次导数符号相反。所以如果一个点是函数的拐点，那么这一点二次导为0.但是反之不成立。二次导为0的地方不一定是函数的拐点。
导数为0的函数
- 如果函数在某个点函数为0，那么在这点有一条水平切线，函数有可能有：最大值、最小值、水平拐点
- 判断图像的方式
	- 求该点左右两侧的一次导数符号：左正右负，函数开口向下，有局部最大值；左负右正，函数开头向上；左右符号相同，则该点为水平拐点。
	- 利用二阶导：该点导数值为0，二阶导为正，函数开口向上，有局部最小值；导数为0，二阶导为负，开口向下，有局部最大值；导数为0，二阶导为0，需要通过导数在该点左右的正负号来判断。

### 0116 - 0119 如何绘制函数图像
1. 判断对称性：是奇函数还是偶函数
2. y轴截距：计算当x=0时的函数值
3. x轴截距
4. 定义域：特别是，当分布为0，分子也为0时，（0，0）为可去不连续点
5. 垂直渐近线：x不存在的地方
6. 函数的正负：函数在0点，不连续点、垂直渐近线附近的正负
7. 水平渐近线：当X趋近正负无穷时函数的取值
8. 导数的正负
9. 通过导数正负判断最大最小值或水平拐点
10. 二次导正负
11. 通过二次导正负判断拐点，同时校验最大最小值